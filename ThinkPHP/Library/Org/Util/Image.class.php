...
class Image {
    ...
    static public function water($source, $water, $savename=null, $alpha=80) {
        ...
        $posY = $sInfo["height"] - 10;
        $posX = $sInfo["width"] - 80;

        $tc = imagecolorallocate($sImage, 236, 236, 251);
        //imagestring($sImage, 80, $posX, $posY, $water, $tc);
        imageTTFText($sImage,10,0,$posX,$posY,$tc,'/Assets/font/arial.ttf',$water);
        //水印结束
        if ($sInfo['type'] == 'png' || $sInfo['type'] == 'gif') {
            imagealphablending($sImage, FALSE); //取消默认的混色模式
            imagesavealpha($sImage, TRUE); //设定保存完整的 alpha 通道信息
        }
        //检查文件是否存在
