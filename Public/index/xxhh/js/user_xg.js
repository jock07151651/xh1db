define(function(require, exports, module) {
    ...
    function search(_this) {
        var key = _this.closest('.search').find('#key').val();
        if (key.length == '') {
            App.alert('搜索关键字不能为空');
            return false;
        }
        window.location = '/joke/search/key/' + key;
    }
    $('#phnav').find('li').click(function() {
    ...
    //右侧排行滚动
    function phslide() {
        if ($('#ulpaihang').length < 1) {
            return ! 1;
        }
        var i = 1;
        setInterval(function() {
            var t = 15 + ( - 55 * i);
            $('#ulpaihang').stop().animate({
                'margin-top': t + 'px'
            },
            800);
            if (i > 2) {
    ...
    //关注 取关
    $('body').delegate('a[data-ajax]', 'click',
    function() {
        if (!user.isLogin()) {
            user.loginDialog();
            return;
        ...
        var _t = $(this);
        var userid = _t.data('userid');
        if (userid) {
            if (!_t.hasClass('user-cancelfollow')) {
                App.request({
                    url: '/ajax/follow',
                    data: {
                        uid: userid
                    ...
                    success: function(re) {
                        if (re.err) {
                            _t.addClass('user-cancelfollow').html('<i class="icon icon10 icon-cut"></i>已关注');
                        } else {
                            App.alert(re.msg);
                        }
                    }
                ...
                App.request({
                    ...
                    success: function(re) {
                        if (re.err) {
                            _t.removeClass('user-cancelfollow').html('<i class="icon icon10 icon-add-h"></i>关注');
                        } else {
                            App.alert(re.msg);
                        }
                    }
    ...
    function() {
        ...
        function() {
            ...
            if (maxlen - len < 0) {
                ...
                len = maxlen - len;
                dmHtml.find('#maxlen').html(len);
            }
        });
        dmHtml.find('.alert-close').click(function() {
            dmHtml.remove();
        });
        dmHtml.find('#submit_dm').click(function() {
            var content = dmHtml.find('#dmcontent').val();
            if (content == '') {
                App.alert('私信内容不能为空');
                return false;
            }
    ...
    var user = {
        ...
        jokeList: function() {
            ...
            listBox.each(function(i, v) {
                        ...
                        App.request({
                            ...
                            success: function(result) {
                                ...
                                if (re.err) {
                                    ...
                                    mainList.find('.joke-list-comment .comment_packUp').on('click',
                                    function() {
                                        $(this).parents('.joke-list-comment').hide();
                                        flag = 1;
                                    });
                                    mainList.find('.loginClickHref').on('click',
                                    function() {
                                        _this.loginDialog();
                                    });
                                    mainList.find('.comment-submit').on('click',
                                    function() {
                                    ...
                                    mainList.on('click', '.comment-ding-icon',
                                    function() {
                                        var self = $(this),
                                        id = self.data('id');
                                        _this.commentDing(self, id);
                                    });
                                    mainList.on('click', '.comment-reply',
                                    function() {
                                        if (!_this.isLogin()) {
                                            _this.loginDialog();
                                            return;
                ...
                detailComment.find('.dingcai').on('click',
                function() {
            ...
            function scroll() {
                ...
                function(i, v) {
                    ...
                    if ($(window).height() + $(window).scrollTop() >= v.offset().top) {
                        var gifPlayBtn = v.parents('.content').find('.gif-play-btn');
                        gifPlayBtn.show();
                        gifPlayBtn.click(function() {
                            ...
                            gifPlayBtn.css({
                                ...
                                'background-image': 'url(/Public/index/qiushibaike/images/loading.gif)'
                            }).html('');
        ...
        funny: function() {
            ...
            function(i, v) {
                ...
                if (h >= 500) {
                    ...
                    v.append('<dd class="joke-content-area"><a href="javascript:void(0)" class="joke-height-h" data-height="' + h + '">展开</a></dd>');
                };
            });
        ...
        register: function() {
            ...
            username.blur(function() {
                ...
                if (form_flag.check_user()) {
                    ...
                    if (username.val() == '') {
                        ...
                        self.next('.error').removeClass('valid').show().html('昵称错误，长度2~12个字符！');
                    };
                };
            });
            sex.click(function() {
            ...
            submit.click(function() {
                ...
                if (!form_flag.check_user()) {
                    ...
                    username.next('.error').removeClass('valid').show().html('昵称错误，长度2~12个字符！');
                    falg = false;
                };
        ...
        setQQ: function(form, url, str) {
            ...
            qq_submit = eidtQQNum.find('.submit'),
            qq_btn = eidtQQNum.find('.btn'),
        ...
        setPortrait: function() {
            ...
            editImg.click(function() {
                ...
                imgDialog.init({
                    w: 800,
                    h: 400,
                    content: '<div class="upload-content"><div class="upload-left"><span class="avatar-symbol">+</span><p style="text-align:center;">点击上传头像，仅支持JPG、GIF、PNG格式，文件小于5M。（高质量图片可生成高清头像）</p></div><div class="upload-right"><div class="preview-wrap" style="overflow:hidden;"><img class="preview preview_img" src="' + editImg.find('img').attr('src') + '"></div></div><form class="imgform"><div class="ajax-file-upload"><input type="file" id="file" name="file" /></div><input type="button" value="保存并关闭" class="imgsubmit"></form></div>',
                    cb: function() {
                        App.uploadify($('#file'),
                        function(imageData) {
                            ...
                            require.async(['/Public/js/imgareaselect/jquery.imgareaselect.min', '/Public/js/imgareaselect/css/imgareaselect-default.css'],
                            function() {
        ...
        sign: function() {
            var _this = this;
            $(document).delegate('#user-sign', 'click',
            function() {
                if (!_this.isLogin()) {
                    _this.loginDialog();
                    return;
